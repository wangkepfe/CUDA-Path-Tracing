...
__device__ inline Vec3f fresnelShlick(Vec3f& F0, float cosTheta) {
    float e = 1.0f - cosTheta;
    float e2 = e * e;
    return F0 + (Vec3f(1.0f) - F0) * e2 * e2 * e;
}

__device__ inline float fresnelShlick(float F0, float cosTheta) {
    float e = 1.0f - cosTheta;
    float e2 = e * e;
    return F0 + (1.0f - F0) * e2 * e2 * e;
}
...
__device__ inline void macrofacetReflection (
    ...
    Vec3f& beta, Vec3f& F0,
    float alphax, float alphay, float kd, float ks)
{
    // isotropic
    ...
    float G;
    float cosThetaWo = dot(nextdir, normal);
...
__device__ inline void roughGlass (
    ...
    float alpha)
{
    ...
    Vec3f n = sampledNormalLocal.x * tangent + sampledNormalLocal.z * bitangent + sampledNormalLocal.y * normal;
    n.normalize();
